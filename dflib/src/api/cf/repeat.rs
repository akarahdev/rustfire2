...
impl Repeat {
    ...
    pub fn for_each<T: VarItem, F: FnOnce(T)>(list: List<T>, code: F) {
        ...
        push_block(TemplateBlock::repeat(
            ...
            ChestArgs::new()
                ...
                .with_slot(26, Item::block_tag("True", "Allow List Changes",
                                               "ForEach", BlockType::Repeat)),
        ));
        push_block(TemplateBlock::bracket(BracketDirection::Start, BracketType::Sticky));
    ...
    pub fn for_each_entry<K: VarItem, V: VarItem, F: FnOnce(K, V)>(dict: Dictionary<K, V>, code: F) {
